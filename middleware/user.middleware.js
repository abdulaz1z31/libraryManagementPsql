...
export const validationUserData = async (req, res, next) => {
    ...
    const userUsername = await pool.query("select id from users where username = $1", [username])
    const email_check = await pool.query("select id from users where email = $1", [email])
